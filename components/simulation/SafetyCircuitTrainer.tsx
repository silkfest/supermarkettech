...
// ── Safety Circuit Trainer — Copeland Discus recip control circuit ─────────────
...
// Two variants:
//   120 V — L1 to neutral. Neutral is grounded, so "to ground" and "to N" agree.
//   208 V single-phase — L1 to L2, both legs hot (~120 V to ground, 208 V between),
//           with a second fuse on the L2 leg. To-ground readings can't find an open.
// Two modes:
...
export type CircuitVariant = '120' | '208'
...
const BASE_ELEMENTS: CircuitElement[] = [
  { id: 'fuse', name: 'Control Fuse', short: 'FU', kind: 'fuse', spec: '2 A',
    faultLabel: 'Control fuse blown',
    reveal: 'A blown control fuse kills the whole string — line voltage on the line side, nothing after. Find WHY it blew (shorted coil? chafed wire?) before replacing it.' },
  ...
  { id: 'wire', name: 'Interconnect Wire (T5→T6)', short: 'WIRE', kind: 'wire', spec: 'panel wire pull',
    ...
    reveal: 'Every device tests fine and the circuit is still dead — the break is in the WIRE. Loose lugs and chafed pulls in the corner of the panel are classic. The meter finds it: line voltage on one end of the run, 0 V on the other.' },
  ...
  { id: 'coil', name: 'Contactor Coil (M)', short: 'M', kind: 'coil', spec: '120 V',
    ...
    reveal: 'Full line voltage across the coil and the contactor still won\'t pull in — the coil is open. Careful: a healthy ENERGIZED coil also reads line voltage across it; the difference is whether the comp is running.' },
]

const FU2: CircuitElement = {
  id: 'fuse2', name: 'L2 Control Fuse', short: 'FU2', kind: 'fuse', spec: '2 A · L2 leg',
  faultLabel: 'L2-leg control fuse blown',
  reveal: 'The trap on 208 V single-phase. Every point on the string reads 208 V to L2 — even the far side of the coil — because nothing is dropping voltage. The open is in the RETURN leg. Across the coil: 0 V. Across FU2: 208 V. And to ground, everything reads ~120 V and tells you nothing.',
}

// Snake layout: 3 rows. Node i sits upstream of element i.
const BASE_NODES: { x: number; y: number; label: string }[] = [
  { x: 28,  y: 55,  label: 'L1' },
  { x: 130, y: 55,  label: 'T1' },
  { x: 230, y: 55,  label: 'T2' },
  { x: 330, y: 55,  label: 'T3' },
...
type Span = { inPath: [number, number][]; outPath: [number, number][]; sym: [number, number] }
...
const FU2_SPAN: Span = { inPath: [[330, 215], [350, 215]], outPath: [[382, 215], [402, 215]], sym: [366, 215] }

type Rail = 'L1' | 'L2' | 'N' | 'GND'
// Single-phase 208 is two legs of a wye, 120° apart: 120 V each to ground, 208 V between.
const PHASOR: Record<Rail, [number, number]> = {
  L1: [120, 0],
  L2: [120 * Math.cos((2 * Math.PI) / 3), 120 * Math.sin((2 * Math.PI) / 3)],
  N: [0, 0],
  GND: [0, 0],
}
function volts(a: Rail, b: Rail): number {
  const [ax, ay] = PHASOR[a], [bx, by] = PHASOR[b]
  return Math.round(Math.hypot(ax - bx, ay - by))
}

const HOT  = '#f59e0b'   // on L1
const HOT2 = '#f97316'   // on L2 (208 variant) — live, just the other leg
const DEAD = '#94a3b8'
const IDLE = '#64748b'
...
export default function SafetyCircuitTrainer({ variant = '120', onSolved }: Props) {
  const is208 = variant === '208'
  const ELEMENTS = is208 ? [...BASE_ELEMENTS, FU2] : BASE_ELEMENTS
  const SPANS = is208 ? [...BASE_SPANS, FU2_SPAN] : BASE_SPANS
  const returnRail: Rail = is208 ? 'L2' : 'N'
  const NODES = is208
    ? [...BASE_NODES, { x: 330, y: 215, label: 'T9' }, { x: 402, y: 215, label: 'L2' }]
    : [...BASE_NODES, { x: 330, y: 215, label: 'N' }]
  const RETURN_IDX = NODES.length - 1
  const GND_IDX = NODES.length          // ground test point — off the string
  const GND_POS = { x: 402, y: 100 }
  const coilIdx = ELEMENTS.findIndex(e => e.kind === 'coil')
  ...
  // A variant switch resets the board so a fault index from the other list can't linger.
  useEffect(() => {
    setPracticeFault(-1); setMysteryFault(null)
  ...
  /** Which rail a test point sits on with a single open at `openIdx` (−1 = healthy: the coil drops the voltage). */
  function railOf(node: number, openIdx: number): Rail {
    if (node === GND_IDX) return 'GND'
    if (node >= RETURN_IDX) return returnRail
    const eff = openIdx === -1 ? coilIdx : openIdx
    return node <= eff ? 'L1' : returnRail
  }
  ...
  function confirmDiagnosis() {
    ...
    if (pickedElement === mysteryFault) {
      ...
      saveSimAttempt({
        rack: 'safety-circuit', scenarioId: is208 ? 'safety-circuit-208' : 'safety-circuit',
        scenarioName: `Safety Circuit ${is208 ? '208 V' : '120 V'} — Find the Fault`, difficulty: is208 ? 'Advanced' : 'Intermediate',
  ...
  const reading = probes.red !== null && probes.black !== null
    ? volts(railOf(probes.red, activeFault), railOf(probes.black, activeFault))
    : null

  const segColor = (elementIdx: number, half: 'in' | 'out') => {
    if (!showVoltageColors) return IDLE
    const node = half === 'in' ? elementIdx : elementIdx + 1
    const rail = railOf(node, activeFault)
    return rail === 'L1' ? HOT : rail === 'L2' ? HOT2 : DEAD
  }

  const probeLabel = (i: number | null) => i === null ? '—' : i === GND_IDX ? 'GND' : NODES[i].label
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300">
          {is208 ? '208 V 1Ø · L1–L2' : '120 V · L1–N'}
        </span>
        {mode === 'mystery' && (
      ...
      <div className="max-w-2xl">
        <svg viewBox="0 0 430 288" className="w-full h-auto select-none" role="img" aria-label={`C1 safety circuit ladder diagram, ${is208 ? '208 V single-phase' : '120 V'}`}>
          ...
          {is208 ? (
            <>
              <text x={402} y={238} textAnchor="middle" fontSize={12} fontWeight={800} fill="#ea580c">L2</text>
              <line x1={402} y1={215} x2={402} y2={226} stroke="#ea580c" strokeWidth={2.5} />
            </>
          ) : (
            <>
          ...
          {/* Ground test point — the cabinet, not part of the string */}
          <g onClick={() => tapNode(GND_IDX)} style={{ cursor: 'pointer' }}>
            <circle cx={GND_POS.x} cy={GND_POS.y} r={13} fill="transparent" />
            <circle cx={GND_POS.x} cy={GND_POS.y} r={5.5}
              fill={probes.red === GND_IDX ? '#dc2626' : probes.black === GND_IDX ? '#1e293b' : '#e2e8f0'}
              stroke={probes.red === GND_IDX ? '#dc2626' : probes.black === GND_IDX ? '#0f172a' : '#16a34a'} strokeWidth={1.6} />
            {probes.black === GND_IDX && <circle cx={GND_POS.x} cy={GND_POS.y} r={2.2} fill="#e2e8f0" />}
            <line x1={GND_POS.x} y1={GND_POS.y + 6} x2={GND_POS.x} y2={GND_POS.y + 14} stroke="#16a34a" strokeWidth={2} />
            <line x1={GND_POS.x - 8} y1={GND_POS.y + 14} x2={GND_POS.x + 8} y2={GND_POS.y + 14} stroke="#16a34a" strokeWidth={2} />
            <line x1={GND_POS.x - 5} y1={GND_POS.y + 18} x2={GND_POS.x + 5} y2={GND_POS.y + 18} stroke="#16a34a" strokeWidth={2} />
            <line x1={GND_POS.x - 2} y1={GND_POS.y + 22} x2={GND_POS.x + 2} y2={GND_POS.y + 22} stroke="#16a34a" strokeWidth={2} />
            <text x={GND_POS.x} y={GND_POS.y - 11} textAnchor="middle" fontSize={9.5} fontWeight={700} fill="#16a34a">GND</text>
          </g>
        </svg>
      </div>
      ...
      {is208 ? (
        <p className="text-[10px] text-slate-400 dark:text-slate-500 leading-relaxed">
          208 V single-phase: both legs are hot. Every live point reads <strong>~120 V to ground</strong> whether the circuit
          is complete or not — so a to-ground hopscotch finds nothing. Put the black probe on <strong>L2</strong> and walk the
          red probe L1 → T9: points read 208 V until you cross the open device. If <em>everything</em> reads 208 V to L2 and
          the comp still won&apos;t run, the open is in the L2 leg (FU2) — confirm with 0 V across the coil and 208 V across FU2.
        </p>
      ) : (
        <p className="text-[10px] text-slate-400 dark:text-slate-500 leading-relaxed">
          ...
          Neutral is bonded to ground here, so reading to <strong>GND</strong> gives the same answer as reading to N.
        </p>
      )}
    </div>
  )

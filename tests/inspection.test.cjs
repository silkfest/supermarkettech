...
function fixture() {
  ...
  return {
    ...
    record() { for (const e of [...this.call.inspection.evidence]) this.act({ type: 'record', id: `${e.phase}:${e.id}:${e.atMin}` }); return this },
...
test('complete observe → measure → repair → verify → score service path', () => {
  const f = fixture()
  f.observe('coil').observe('controller').sample('product').record()
  f.hands('open-cover').act({ type: 'force-defrost', tool: 'controller' }).sample('current').record().wait(8).observe('coil').record()
  ...
  assert.equal(canDiagnose(f.call.inspection), false, 'observed evidence must be recorded')
  f.record()
  ...
  f.hands('restore').act({ type: 'force-defrost', tool: 'controller' }).hands('open-cover').sample('current').record().wait(12)
  f.observe('coil').observe('controller').record()
  assert.ok(f.call.inspection.frost.every(n => n <= 8))
  assert.notEqual(f.call.inspection.terminatedAt, null)
  f.wait(15).wait(15).sample('product').record().hands('close-cover')
...
test('closing/reopening inspection preserves evidence on the active call; JSON round-trip works', () => {
  const f = fixture().observe('drain').record()
...
test('repeated samples preserve measured temperature history in the notebook', () => {
  const f = fixture().sample('product').record().wait(10).sample('product').record()
